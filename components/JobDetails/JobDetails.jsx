  ...
  return (
    <div className="max-w-4xl mx-auto bg-white shadow-lg rounded-xl p-6 border border-gray-200">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-4 text-gray-600">
        <p className="flex items-center gap-2"><Briefcase size={18} /> {job.sector}</p>
        <p className="flex items-center gap-2"><MapPin size={18} /> {job.location}</p>
        <p className="flex items-center gap-2"><DollarSign size={18} /> {job.salary}</p>
        <p className="flex items-center gap-2"><Calendar size={18} /> Deadline: {new Date(job.deadline).toLocaleDateString()}</p>
        <p className="flex items-center gap-2"><Users size={18} /> Applicants Needed: {job.requiredApplicants}</p>
      </div>
      ...
      <div className="mt-6 text-center">
        <button
          ...
          className="bg-blue-600 text-white text-lg font-medium px-6 py-3 rounded-lg shadow-md hover:bg-blue-700 transition duration-300"
        >
          Apply Now
        </button>
      </div>
